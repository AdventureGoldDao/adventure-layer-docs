...
- **Shard Node:**
  - URL: `http://rpc.adventure.dev/{node}/shard`
  - Description: In the context of The Adventure Layer, {node} serves as a placeholder for accessing specific nodes within the blockchain infrastructure. Depending on the context, replace {node} with the actual node identifier, such as node1. This identifier helps direct requests to the shard node, enabling targeted interactions tailored to the node's specialized functionalities.
