---
title: Welcome to the Adventure Layer Docs
lang: en-US
description: Learn about the Adventure Layer Collective, the Adventure Layer Docs, and how to get started building with Optimism.
---

import { Cards, Card } from 'nextra/components'

# Welcome to the Adventure Layer Docs

Welcome to the Adventure Layer Docs, the unified home of the [Adventure Layer Collective's](/connect/resources/glossary#optimism-collective) technical documentation and information about the [OP Stack](/stack/getting-started).
Information about the Adventure Layer Collective's governance, community, and mission can be found on the [Adventure Layer Community Hub](https://community.optimism.io/docs/governance/).

## Guides for Builders

Whether you're a developer building a app on Adventure Layer Testnet, a node operator running an Adventure Layer Testnet node, or a chain operator launching your own OP Stack chain, you'll find everything you need to get started right here.

<Cards>
  <Card title="App Developers" href="/builders/app-developers/overview" icon={<img src="img/icons/terminal-window-line.svg" />} />

  <Card title="Chain Operators" href="/builders/chain-operators/self-hosted" icon={<img src="img/icons/instance-line.svg" />} />

  <Card title="Node Operators" href="/builders/node-operators/rollup-node" icon={<img src="img/icons/computer-line.svg" />} />

  <Card title="Wallets & CEXs" href="/builders/cex-wallet-developers/wallet-support" icon={<img src="img/icons/wallet.svg" />} />

  <Card title="Developer Tools" href="/builders/tools/overview" icon={<img src="img/icons/tools.svg" />} />
</Cards>

## Featured Tools

Check out these amazing tools, so you can get building with Optimism.
<link rel="stylesheet" href="/integrations/custom/style.css" />

<Cards>
  <Card title="Superchain Faucet" href="https://app.optimism.io/faucet?utm_source=docs" icon={<img src="img/icons/gear.svg" />} />

  <Card title="Scaffold-OP" href="/builders/app-developers/quick-start" icon={<img src="img/icons/gear.svg" />} />

  <Card title="Superchain Dev Console" href="https://console.optimism.io/?utm_source=docs" icon={<img src="img/icons/gear.svg" />} />

  <Card title="Open-Source Repo" href="/builders/app-developers/tools/ecosystem-overview" icon={<img src="img/icons/gear.svg" />} />
</Cards>

## Learn About Optimism

Adventure Layer Testnet is an [EVM equivalent](https://web.archive.org/web/20231127160757/https://medium.com/ethereum-optimism/introducing-evm-equivalence-5c2021deb306) Layer 2 blockchain connected to Ethereum.
The OP Stack is the standardized, shared, and open-source development stack that makes it easy to spin up your own production-ready Layer 2 blockchain just like Adventure Layer Testnet.
The Superchain is a network of OP Stack chains that share a bridging protocol, governance system, and more.
We've got you covered with these detailed guides to help you learn all about Optimism's tech stack.

<Cards>
  <Card title="About Adventure Layer Testnet" href="/chain/getting-started" icon={<img src="img/icons/book.svg" />} />

  <Card title="About the OP Stack" href="/stack/getting-started" icon={<img src="img/icons/book.svg" />} />

  <Card title="About the Superchain" href="/stack/explainer" icon={<img src="img/icons/book.svg" />} />
</Cards>
