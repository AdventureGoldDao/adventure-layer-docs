...
Modifications to the OP Stack may prevent a chain from being able to benefit from aspects of the [Adventure Layer Superchain](/stack/explainer).
...
You'll be using the L1 Bedrock smart contracts found in the [`contracts-bedrock` package](https://github.com/ethereum-optimism/optimism/tree/op-contracts/v1.0.0/packages/contracts-bedrock) within the [Adventure Layer Monorepo](https://github.com/ethereum-optimism/optimism).
...
Several implementations of the OP Stack consensus client exist, including `op-node` (maintained by Adventure Layer Foundation), [`magi`](https://github.com/a16z/magi) (maintained by a16z) and [`hildr`](https://github.com/optimism-java/hildr) (maintained by OptimismJ).
In this tutorial you'll be using the [`op-node` implementation](https://github.com/ethereum-optimism/optimism/tree/v1.1.4/op-node) found within the [Adventure Layer Monorepo](https://github.com/ethereum-optimism/optimism).
...
Various implementations of the OP Stack execution client exist, including `op-geth` (maintained by Adventure Layer Foundation), [`op-erigon`](https://github.com/testinprod-io/op-erigon) (maintained by Test in Prod), and `op-nethermind` (coming soon).
...
You'll be using the [`op-batcher` implementation](https://github.com/ethereum-optimism/optimism/tree/v1.1.4/op-batcher) of the Batcher component found within the [Adventure Layer Monorepo](https://github.com/ethereum-optimism/optimism).
...
You'll be using the [`op-proposer` implementation](https://github.com/ethereum-optimism/optimism/tree/v1.1.4/op-proposer) of the Proposer component found within the [Adventure Layer Monorepo](https://github.com/ethereum-optimism/optimism).
...
The OP Stack Rollups were designed to use EVM Equivalent blockchains like Ethereum, Adventure Layer Testnet, or standard Ethereum testnets as their L1 chains.
...
{<h3>Enter the Adventure Layer Monorepo</h3>}
...
<Callout>
You will be using the `tutorials/chain` branch of the Adventure Layer Monorepo to deploy an OP Stack testnet chain during this tutorial.
...
{<h3>Build the various packages inside of the Adventure Layer Monorepo</h3>}
...
Once you've built both repositories, you'll need to head back to the Adventure Layer Monorepo to set up the configuration file for your chain.
...
**You can generate a new `IMPL_SALT` by running `direnv allow` anywhere in the Adventure Layer Monorepo.**
</Callout>
