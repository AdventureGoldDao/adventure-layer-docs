...
Ticking Adventure Layer is a proof-of-concept implementation of an OP Stack chain that calls a `tick` function every block. By using the OP Stack, Ticking Adventure Layer avoids the need for off-chain infrastructure to execute a function on a regular basis. Ticking Conway is a system that uses Ticking Adventure Layer to build [Conway's Game of Life](https://conwaylife.com/) onchain.
...
*   [Ticking Adventure Layer on GitHub](https://github.com/therealbytes/ticking-optimism)
