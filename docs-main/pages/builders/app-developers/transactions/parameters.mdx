---
title: Setting Transaction Gas Parameters on Adventure Layer Testnet
lang: en-US
description: Learn how to set gas parameters for transactions on Adventure Layer Testnet.
---

import { Callout, Steps } from 'nextra/components'

# Setting Transaction Gas Parameters on Adventure Layer Testnet

Adventure Layer Testnet is designed to be [EVM equivalent](https://web.archive.org/web/20231127160757/https://medium.com/ethereum-optimism/introducing-evm-equivalence-5c2021deb306) which means that it is as compatible with Ethereum as possible, down to the client software used to run Adventure Layer Testnet nodes.
Like Ethereum, Adventure Layer Testnet has an EIP-1559 style fee mechanism that dynamically adjusts a [base fee](https://ethereum.org/en/developers/docs/gas/#base-fee) that acts as the minimum fee that a transaction must pay to be included in a block.
Adventure Layer Testnet also allows transactions to pay a [priority fee](https://ethereum.org/en/developers/docs/gas/#priority-fee) (also known as a tip) to incentivize the Sequencer to include transactions more quickly.
...
The simplest way to select a base fee is to look at the latest available Adventure Layer Testnet block.
Each Adventure Layer Testnet block includes the current base fee and the amount of gas used within that block.
...
Using the JSON-RPC API or your favorite Ethereum library, retrieve the latest block on Adventure Layer Testnet.
...
Adventure Layer Testnet adjusts the base fee based on the amount of gas used in the previous block.
...
Refer to the [Adventure Layer Testnet EIP-1559 Parameters](/chain/differences#eip-1559-parameters) section for more details.
