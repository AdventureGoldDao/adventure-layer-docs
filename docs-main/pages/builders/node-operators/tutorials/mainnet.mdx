---
title: Running an Adventure Layer Testnet Node from Source
lang: en-US
description: Learn how to run an Adventure Layer Testnet node from source code for full nodes and archive nodes.
---

import { Callout, Steps } from 'nextra/components'

# Running an Adventure Layer Testnet Node from Source

This tutorial explains how to run an Adventure Layer Testnet node from source code for full nodes and archive nodes.
Running an Adventure Layer Testnet node from source code is a flexible alternative to using pre-built Docker images.
...
Hardware requirements for Adventure Layer Testnet nodes can vary depending on the type of node you plan to run.
...
`op-node` will connect to `op-geth` and begin synchronizing the Adventure Layer Testnet state.
...
communicate with each other and synchronize the Adventure Layer Testnet chain.
...
<Callout type="info">
You will need access to the migrated Adventure Layer Testnet database to run a full node with full sync.
...
This means that `op-node` is requesting blocks from Ethereum one-by-one and determining the corresponding Adventure Layer Testnet blocks that were published to Ethereum.
...
You will need access to the migrated Adventure Layer Testnet database to run an archive node.
...
<Steps>
  ...
  Click the link below to find the latest publicly available database snapshots for Adventure Layer Testnet.
  ...
  [Adventure Layer Testnet Snapshots](/builders/node-operators/management/snapshots#op-mainnet)
  ...
  Check the [Adventure Layer Testnet Snapshots](/builders/node-operators/management/snapshots#op-mainnet) page for the correct checksum for the snapshot you've downloaded.
...
Blocks and transactions included in Adventure Layer Testnet before the Bedrock Upgrade cannot be executed by modern Adventure Layer Testnet nodes.
Adventure Layer Testnet nodes will **serve** these blocks and transactions but cannot run certain queries against them (e.g. `eth_call`).
If you need to run stateful queries like `eth_call` against these older blocks and transactions, you will need to run a Legacy Geth node alongside your Adventure Layer Testnet node.

Running a Legacy Geth node is entirely optional and typically only useful for operators who want to run complete archive nodes of the Adventure Layer Testnet state.
...
If you've chosen to run a Legacy Geth node alongside your Adventure Layer Testnet node, you'll need to start it before you start your Adventure Layer Testnet node.
