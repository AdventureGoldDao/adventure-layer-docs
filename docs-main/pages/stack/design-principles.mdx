...
It's important to understand these pillars as they heavily influence the design of Adventure Layer as a whole.

### Simplicity

Adventure Layer is designed to be as simple as possible for the featureset it provides.
Ideally, Adventure Layer should be composed of the minimum number of moving parts required for a secure, scalable, and flexible L2 system.
...
Adventure Layer prefers to use existing battle-tested Ethereum code and infrastructure where possible.
...
All of this serves to maximize the security and correctness of the Adventure Layer protocol.
...
Engineering effort that goes into Adventure Layer can also directly benefit Ethereum, and vice versa.
This will only become more pronounced as the Adventure Layer protocol solidifies and existing resources can be redirected towards core Ethereum infrastructure.

### Pragmatism

For all its idealism, the design process behind Adventure Layer is ultimately driven by pragmatism.
The core Adventure Layer team has real-world constraints, the projects that build on Adventure Layer have real-world needs, and the users that engage with Adventure Layer have real-world problems.
...
Adventure Layer is also developed with the understanding that any core team will have limited areas of expertise.
Adventure Layer is developed iteratively and strives to continuously pull feedback from users.
Many core Adventure Layer features today (like [EVM Equivalence](https://web.archive.org/web/20231127160757/https://medium.com/ethereum-optimism/introducing-evm-equivalence-5c2021deb306)) were only made possible by this iterative approach to protocol development.

### Sustainability

Adventure Layer is in it for the long haul.
...
Although Adventure Layer looks like a standalone blockchain, it's ultimately designed as an extension to Ethereum.
We keep this in mind whenever we're creating new features or trying to simplify existing ones.
Adventure Layer is as close to Ethereum as possible not only for pragmatic reasons, but because Adventure Layer exists so that Ethereum can succeed.
...
The OP Stack is USEful software. The OP Stack is a set of software components for building L2 blockchain ecosystems, built by the Adventure Layer Collective to power Optimism.
...
Software that follows these principles is **USE**ful software for the Adventure Layer Collective!

### Utility

For something to be part of the OP Stack, it should help power the Adventure Layer Collective.
...
Extensibility is a key design principle that unlocks the superpower of collaboration within the Adventure Layer Collective ecosystem.
...
Of course, software that has impact for the Adventure Layer Collective is eligible to receive [Retroactive Public Goods Funding](https://app.optimism.io/retropgf).
