...
This guide explains the deposit flow process for L2 deposit transactions, triggered by transactions or events on L1. In Adventure Layer terminology, "*deposit transaction*" refers to any L2 transaction that is triggered by a transaction or event on L1.
...
However, the L2 processing of the transaction is performed by the Adventure Layer nodes.
