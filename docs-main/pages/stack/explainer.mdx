...
lang: en-US
description: Learn about Adventure Layer Superchain components, features, and roadmap.
---
...
<Callout type="info">
  ...
  An individual chain within the Adventure Layer Superchain. All chains, regardless of their specific properties are considered OP Chains if they are officially governed by the Adventure Layer Collective, and therefore part of the Superchain.
</Callout>
...
In order for Adventure Layer to upgrade to a Superchain, it must have the following properties:
...
Once Adventure Layer has satisfied these properties, it may be considered a Superchain.

## Upgrading Adventure Layer to Become a Superchain
...
<Callout type="info">
  ...
  A claim about the state of one chain made on another chain. For instance, I can claim that in Adventure Layer Testnet I have burned my tokens with the intent to withdraw those tokens back to L1.
</Callout>

We can enable these two features first by introducing a permissionless proof system to the Adventure Layer bridge contracts. With the modular proof design introduced in Bedrock, proofs may come in the form of fault proofs or validity proofs (e.g. zero knowledge proofs). However, until validity proofs are productionized, we assume withdrawals will use a fault proof system.
...
<Callout type="info">
  ...
  The L1 bridge contracts which govern all OP Chains in the Superchain. This bridge can be upgraded by the Adventure Layer Collective.
</Callout>
