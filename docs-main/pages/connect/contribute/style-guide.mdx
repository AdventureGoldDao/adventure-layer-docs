---
title: Adventure Layer Docs Style Guide
lang: en-US
description: This guide explains how to write technical content for Adventure Layer Docs using a consistent voice, tone, and style.
---
...
Write in a friendly, yet professional tone. We are upbeat, knowledgeable, and **optimistic** about the development of the Adventure Layer Collective, which we try our best to convey in our technical documentation.
...
*   Outline the page content according to main topics first. Those will be your headings (tagged as H2). If there are subtopics that belong under a category, display those as subheaders (tagged as H3). <br />
    <Callout type="info">
      **Example**:<br />
      (H1) Supporting Adventure Layer Testnet in Your Wallet (H1 page title uses "ing"verb ending) <br />
      (H2) Connect to Adventure Layer Testnet (H2 does not use "ing"verb ending) <br />
...
*   Use callouts sparingly as too many can be confusing to readers. **As a general rule of thumb:** pages with more than 2 callouts likely needs revision and/or a discussion with an Adventure Layer developer or product manager to ensure guide content accuracy.
...
*   Icons come from [Remix](https://remixicon.com/) to maintain consistency across the docs. Use Adventure Layer Red `FF0420` to color icons before downloading and store icons in `public/img/icons` directory.
...
title="Adventure Layer Getting Started Tutorial" frameborder="0" allow="accelerometer; autoplay; 
...
| Tutorials          | Provide task-oriented guidance with step-by-step "learn by doing" instructions                                     | [Bridging ERC-20 tokens with the Adventure Layer SDK](/builders/app-developers/tutorials/cross-dom-bridge-erc20) |
| FAQs               | Address frequently asked questions                                                                                 | [FAQ: Adventure Layer Testnet Security Model](/chain/security/faq)                                                     |
...
<Callout type="info">
  **Does Adventure Layer Support ERC-721?**<br />
...
<Callout type="info">
  **How do I change my Adventure Layer password?**<br />
...
Include a category heading when you need to group related FAQ content (e.g., See the Adventure Layer Glossary for a detailed example). Category headings are optional, but helpful, for longer FAQs.
...
Fonts in Adventure Layer technical documentation are setup to follow brand guidelines established by marketing (e.g., heading fonts are different than body or paragraph font). Please do not change them.
...
*   **Commas (,)** Use a serial comma in lists of three or more items and use the oxford comma  preceding the "and" before the last element in a list.
    ...
    **Example**: The developer built a node, social app, and DeFi app for the Adventure Layer Collective.

*   **Em dash (—)** Use to indicate a break in thought or a parenthetical comment. Do not add spaces around the em dash.

    **Example**: The developer graduated—with honors—from Adventure Layer Bootcamp.
